...
main_df = pd.DataFrame(columns=['region', 'creation_time', 'user_email', 'job_type', 'statement_type', 'total_bytes_processed', 'total_slot_ms', 'cache_hit', 'total_bytes_billed'])

if regions:
    for region in regions:  # API request(s)

        query_job = client.query("select creation_time, user_email, job_type, statement_type, total_bytes_processed, total_slot_ms, cache_hit, total_bytes_billed/pow(1024,3) as total_gb_billed from "+region+".INFORMATION_SCHEMA.JOBS")
